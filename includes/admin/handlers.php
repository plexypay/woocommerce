 ...
 *
 */
class WC_FOROPAY_Payments_Order_Handler extends WC_FOROPAY_Payments_Gateway {
    ...
    public function capture_payment( $order_id, $previous_status, $next_status ) {
        ...
        if (
            $order->get_meta('foropay_status', true) != 'authorized' ||
        ...
        try {
            ...
            $result = $this->foropay->capture_transaction(
            ...
            $order->add_order_note( sprintf(__( 'Foropay transaction complete (Transaction ID: %s)', 'woocommerce-gateway-foropay' ), $result['id']) );
            $order->update_meta_data( 'foropay_status', 'charged' );
...
new WC_FOROPAY_Payments_Order_Handler();
